...
static inline void fromRawValue(const RawValue &value, FastImageViewPriority &result) {
...
static inline void fromRawValue(const RawValue &value, FastImageViewCache &result) {
  auto string = (std::string)value;
...
static inline void fromRawValue(const RawValue &value, FastImageViewResizeMode &result) {
  auto string = (std::string)value;
...
static inline void fromRawValue(const RawValue &value, FastImageViewSourceHeadersStruct &result) {
  ...
  if (tmp_value != map.end()) {
    fromRawValue(tmp_value->second, result.value);
  }
}
...
static inline void fromRawValue(const RawValue &value, std::vector<FastImageViewSourceHeadersStruct> &result) {
...
static inline void fromRawValue(const RawValue &value, FastImageViewSourceStruct &result) {
  ...
  if (tmp_headers != map.end()) {
    fromRawValue(tmp_headers->second, result.headers);
  }
  auto tmp_priority = map.find("priority");
  if (tmp_priority != map.end()) {
    fromRawValue(tmp_priority->second, result.priority);
  }
  auto tmp_cache = map.find("cache");
  if (tmp_cache != map.end()) {
    fromRawValue(tmp_cache->second, result.cache);
  }
}
...
class FastImageViewProps final : public ViewProps {
 public:
  ...
  FastImageViewProps(const FastImageViewProps &sourceProps, const RawProps &rawProps);
